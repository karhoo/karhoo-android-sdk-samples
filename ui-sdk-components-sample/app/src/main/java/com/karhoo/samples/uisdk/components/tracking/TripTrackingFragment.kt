package com.karhoo.samples.uisdk.components.tracking
...
import com.karhoo.samples.uisdk.components.utils.CurrencyUtils
...
class TripTrackingFragment : BaseFragment() {
    ...
    private fun bindViewTripInfo(trip: TripInfo) {
        ...
        selected_drop_off?.text = trip.destination?.displayAddress
    ...
    private fun observeDriverPosition(tripIdentifier: String) {
        ...
        driverPositionObserver?.let {
            driverTrackingInfoObservable?.subscribe(it,
                TRIP_INFO_UPDATE_PERIOD
            )
        }
    }
    ...
    companion object {
        ...
        fun newInstance(owner: FragmentActivity,
                        bookingRequestStateViewModel: BookingRequestStateViewModel) = TripTrackingFragment()
            .apply {
